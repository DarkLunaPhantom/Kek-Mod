...
static bool concealUnknownCivs() // DarkLunaPhantom - Require debug mode. Conceal while in advanced start.
{
	return GC.getGameINLINE().getActiveTeam() != NO_TEAM && !GC.getGameINLINE().isDebugMode() && (!gDLL->GetWorldBuilderMode() || gDLL->getInterfaceIFace()->isInAdvancedStart());
}
...
const wchar* CvPlayer::getName(uint uiForm) const
{
	if (GC.getInitCore().getLeaderName(getID(), uiForm).empty() || (GC.getGameINLINE().isMPOption(MPOPTION_ANONYMOUS) && isAlive() && GC.getGameINLINE().getGameState() == GAMESTATE_ON))
	{
		...
		if (!concealUnknownCivs() || GET_TEAM(GC.getGameINLINE().getActiveTeam()).isHasSeen(getTeam()) || !isAlive()) // DarkLunaPhantom - Don't conceal the name of dead players.
			return GC.getLeaderHeadInfo(getLeaderType()).getDescription(uiForm);
		else
		{
...
const wchar* CvPlayer::getCivilizationDescription(uint uiForm) const
{
	...
	if (concealUnknownCivs() && !GET_TEAM(GC.getGameINLINE().getActiveTeam()).isHasSeen(getTeam()) && isAlive()) // DarkLunaPhantom - Don't conceal the civilization of dead players.
	{
			...
			if (getTeam() != GET_PLAYER(eWhoTo).getTeam() && !GET_TEAM(GET_PLAYER(eWhoTo).getTeam()).isVassal(getTeam()))
			{
				if (!atWar(getTeam(), GET_PLAYER(eWhoTo).getTeam()))
				{
					if (!(GET_TEAM(getTeam()).isDefensivePact(GET_PLAYER(eWhoTo).getTeam())))
					{
						if (GET_TEAM(getTeam()).isDefensivePactTrading() || GET_TEAM(GET_PLAYER(eWhoTo).getTeam()).isDefensivePactTrading())
						{
							// DarkLunaPhantom begin - added possibility of signing defensive pact while in war if BBAI defensive pact option is >= 1
							if (((GET_TEAM(getTeam()).getAtWarCount(true) == 0) && (GET_TEAM(GET_PLAYER(eWhoTo).getTeam()).getAtWarCount(true) == 0)) || GC.getBBAI_DEFENSIVE_PACT_BEHAVIOR() >= 1)
							// DarkLunaPhantom end
							{
...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	if (!bTestVisible)
	{
		// DarkLunaPhantom begin - Changed so that team members do not block players from trying to build limited units.
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreTech) const
{
	...
	if (!bTestVisible)
	{
		// DarkLunaPhantom begin - Changed so that team members do not block players from trying to build limited buildings (wonders).
...
PlayerColorTypes CvPlayer::getPlayerColor() const
{
	...
	if (!concealUnknownCivs() || GET_TEAM(GC.getGameINLINE().getActiveTeam()).isHasSeen(getTeam()) || !isAlive()) // DarkLunaPhantom - Don't conceal the color of dead players.
		return GC.getInitCore().getColor(getID());
	else
	...
	if (kMission.getCounterespionageNumTurns() > 0 && kMission.getCounterespionageMod() > 0)
	{
		...
		if (NO_TEAM != eTargetTeam)
		{
			...
			if( GET_TEAM(getTeam()).getCounterespionageTurnsLeftAgainstTeam(eTargetTeam) == 0 ){
				// PB Mod: Do not increase twice for multiple counter espionage.
