...
/************************************************************************************************/
			...
			for (iI = 0; iI < GC.getNumImprovementInfos(); iI++)
			{
				...
				if( pPlot->canHaveImprovement((ImprovementTypes)iI, pWorkingCity->getTeam()), false, NO_BUILD, true) // DarkLunaPhantom
				{
						...
						for (iI = 0; iI < GC.getNumBuildInfos(); ++iI)
						{
							if (GC.getBuildInfo((BuildTypes) iI).getImprovement() != NO_IMPROVEMENT)
							{
								...
								if (kImprovementInfo.isImprovementBonusTrade(eBonus))
								{
									...
									if (pPlot->canHaveImprovement(((ImprovementTypes)(GC.getBuildInfo((BuildTypes) iI).getImprovement())), GC.getGameINLINE().getActiveTeam(), true, NO_BUILD, true)) // DarkLunaPhantom
									{
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if (isWorldUnitClass(eUnitClass))
	{
		...
		else
		{
			...
			int iRemaining = std::max(0, GC.getUnitClassInfo(eUnitClass).getMaxGlobalInstances() - (ePlayer != NO_PLAYER ? GC.getGameINLINE().getUnitClassCreatedCount(eUnitClass) + GET_PLAYER(ePlayer).getUnitClassMaking(eUnitClass) : 0));
			szBuffer.append(NEWLINE);
	...
	if (isTeamUnitClass(eUnitClass))
	{
		...
		else
		{
			...
			int iRemaining = std::max(0, GC.getUnitClassInfo(eUnitClass).getMaxTeamInstances() - (ePlayer != NO_PLAYER ? GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getUnitClassCount(eUnitClass) + GET_PLAYER(ePlayer).getUnitClassMaking(eUnitClass) : 0));
			szBuffer.append(NEWLINE);
	...
	else
	{
		if (isWorldWonderClass((BuildingClassTypes)(kBuilding.getBuildingClassType())))
		{
			...
			else
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_BUILDING_WORLD_WONDER_LEFT", (GC.getBuildingClassInfo((BuildingClassTypes) kBuilding.getBuildingClassType()).getMaxGlobalInstances() - GC.getGameINLINE().getBuildingClassCreatedCount((BuildingClassTypes)(kBuilding.getBuildingClassType())) - GET_PLAYER(ePlayer).getBuildingClassMaking((BuildingClassTypes)(kBuilding.getBuildingClassType())))));
			}
		}
		...
		if (isTeamWonderClass((BuildingClassTypes)(kBuilding.getBuildingClassType())))
		{
			...
			else
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_BUILDING_TEAM_WONDER_LEFT", (GC.getBuildingClassInfo((BuildingClassTypes) kBuilding.getBuildingClassType()).getMaxTeamInstances() - GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getBuildingClassCount((BuildingClassTypes)(kBuilding.getBuildingClassType())) - GET_PLAYER(ePlayer).getBuildingClassMaking((BuildingClassTypes)(kBuilding.getBuildingClassType())))));
			}
		}
	...
	if (!bCivilopediaText)
	{
		...
		if (isWorldProject(eProject))
		{
			...
			else
			{
				...
				szBuffer.append(gDLL->getText("TXT_KEY_PROJECT_WORLD_NUM_LEFT", (kProject.getMaxGlobalInstances() - GC.getGameINLINE().getProjectCreatedCount(eProject) - GET_PLAYER(ePlayer).getProjectMaking(eProject))));
			}
		}
...
void CvGameTextMgr::setEspionageCostHelp(CvWStringBuffer &szBuffer, EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pSpyUnit)
{
	...
	iMissionCost = (int)(iMissionCost * sqrt((1 + 0.5 * (GET_TEAM(kPlayer.getTeam()).getNumMembers() - 1))/(1 + 0.5 * (GET_TEAM(GET_PLAYER(eTargetPlayer).getTeam()).getNumMembers() - 1))));
